# -*- coding: utf-8-*-
"""
A Speaker handles audio output from Shella to the user
...
class EspeakTTS(AbstractTTSEngine):
    """
    Uses the eSpeak speech synthesizer included in the Shella disk image
...
class MimicTTS(AbstractTTSEngine):
    ...
    def get_config(cls):
        ...
        if os.path.exists(profile_path):
            with open(profile_path, 'r') as f:
                profile = yaml.safe_load(f)
                if 'mimic-tts' in profile:
                    if 'voice' in profile['mimic-tts']:
                        config['voice'] = profile['mimic-tts']['voice']
        return config

    @classmethod
    def is_available(cls):
        return (super(cls, cls).is_available() and
                diagnose.check_executable('mimic') and
                len(cls.get_voices()) > 0)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Shella TTS module')
