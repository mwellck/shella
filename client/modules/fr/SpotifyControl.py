...
WORDS = ["MUSIQUE", "MUSIQUES", "SPOTIFY", "MUSIC"]
...
def handle(text, mic, profile):
    ...
    try:
        ...
        mic.say("Veuillez m'excuser mais il semblerait que Spotify ne soit pas activé." +
                "Merci de lire la documentation concernant Spotify pour plus d'informations.")
        return

    mic.say("Donnez moi un moment, je charge actuellement vos playlists Spotify.")
...
class MusicMode(object):
    ...
    def __init__(self, PERSONA, mic, mpdwrapper):
        ...
        phrases = ["STOP", "FERME", "JOUE", "LANCE", "PLAY", "PAUSE", "SUIVANTE", "PRECEDENTE",
                   "SUIVANT", "PRECEDENT", "AUGMENTE", "REDUIT", "FORT", "FAIBLE", "VOLUME",
                   "ARRETE", "PLAYLIST"]
    ...
    def delegateInput(self, input):
        ...
        elif any(ext in command for ext in ["STOP", "ARRETE"]):
            self.mic.say("Arrêt de la musique")
            self.music.stop()
            return
        elif "PLAY" in command:
            self.mic.say("Joue %s" % self.music.current_song())
            self.music.play()
            return
        elif "PAUSE" in command:
            self.mic.say("Pause de la musique")
            # not pause because would need a way to keep track of pause/play
            # state
            self.music.stop()
            return
        elif any(ext in command for ext in ["AUGMENTE", "FORT"]):
            self.mic.say("Augmentation")
            self.music.volume(interval=10)
            self.music.play()
            return
        elif any(ext in command for ext in ["FAIBLE", "REDUIT"]):
            self.mic.say("Réduction")
            self.music.volume(interval=-10)
            self.music.play()
            return
        elif any(ext in command for ext in ["SUIVANT", "SUIVANTE"]):
            self.mic.say("Musique suivante")
            self.music.play()  # backwards necessary to get mopidy to work
            self.music.next()
            self.mic.say("Lecture de %s" % self.music.current_song())
            return
        elif any(ext in command for ext in ["PRECEDENT", "PRECEDENTE"]):
        ...
        if playlists:
            self.mic.say("Chargement de la playlist %s" % playlists[0])
            self.music.play(playlist_name=playlists[0])
            self.mic.say("Lecture de %s" % self.music.current_song())
        else:
            self.mic.say("Aucune playlist trouvée. Reprise de la lecture en cours.")
            self.music.play()
    ...
    def handleForever(self):
        ...
        while True:
            ...
            if input:
                if "ferme" in input.lower():
                    self.mic.say("Fermeture de Spotify")
                    return
...
class MPDWrapper(object):
    ...
    def current_song(self):
        ...
        result = "%s par %s" % (item["title"], item["artist"])
        return result
