...
# Add shellapath.LIB_PATH to sys.path
sys.path.append(shellapath.LIB_PATH)

parser = argparse.ArgumentParser(description='Shella Voice Control Center')
        ...
        if not os.path.exists(shellapath.CONFIG_PATH):
            try:
                ...
                self._logger.error("Could not create config dir: '%s'",
                                   shellapath.CONFIG_PATH, exc_info=True)
                raise

        # Check if config dir is writable
        if not os.access(shellapath.CONFIG_PATH, os.W_OK):
            self._logger.critical("Config dir %s is not writable. Shella " +
                                  "won't work correctly.",
                                  shellapath.CONFIG_PATH)

        # FIXME: For backwards compatibility, move old config file to newly
        #        created config dir
        old_configfile = os.path.join(shellapath.LIB_PATH, 'profile.yml')
        new_configfile = shellapath.config('profile.yml')
...
if __name__ == "__main__":
    ...
    print("*******************************************************")
    print("*             SHELLA - PERSONNAL ASSISTANT            *")
    ...
    if not args.no_network_check and not diagnose.check_network_connection():
        logger.warning("Network not connected. This may prevent Shella from " +
                       "running properly.")
